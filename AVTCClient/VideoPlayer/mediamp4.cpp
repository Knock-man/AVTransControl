...
int MediaMP4::open(const QUrl& url, uint64_t* fsize)
{
    ...
    file = new QFile(url.path().right(url.path().size() - 1));//创建QFile 对象
    if(file == NULL)
    {
    ...
    if(file->open(QFile::ReadOnly))//以读的方式打开文件
    {
        *fsize = file->size();
        pos = uint16_t(*fsize & 0xFFFF);//文件长度后16位作为偏移
...
//        {
//            //偏移量+指针位置为key的起始点   从起始点不断循环key进行异或解密
