...
string BigNum::base64Encode(const string& buffer, bool newline)
{
    ...
    BIO* b64 = NULL;
    BUF_MEM* bptr;
    b64 = BIO_new(BIO_f_base64());//创建一个Base64 BIO
    if (!newline)
    {
        BIO_set_flags(b64, BIO_FLAGS_BASE64_NO_NL);
    }
    bmem = BIO_new(BIO_s_mem());//创建一个内存BIO
    b64 = BIO_push(b64, bmem);//将 Base64 BIO连接到内存BIO上，以便将编码后的数据写入内存
    BIO_write(b64, buffer.c_str(), (int)buffer.size());//写入数据到BIO链上
    BIO_flush(b64);//刷新BIO链
    BIO_get_mem_ptr(b64, &bptr);//获取内存BIO中的数据指针
    BIO_set_close(b64, BIO_NOCLOSE);
    string buff;
    buff.resize(bptr->length + 1);
    memcpy((char*)buff.c_str(), bptr->data, bptr->length);//复制数据
    buff[bptr->length] = 0;
    BIO_free_all(b64);//释放BIO链
    return buff;
}
...
string BigNum::base64Decode(const string& input, bool newline)
{
    ...
    b64 = BIO_new(BIO_f_base64());//创建一个Base64 BIO过滤器
    if (!newline)
    {
        BIO_set_flags(b64, BIO_FLAGS_BASE64_NO_NL);
    }
